...
class MyMainWindow(QtWidgets.QMainWindow, gui.Ui_MainWindow):
    ...
    def add_alarm(self):
        try:
            ...
            if is_range and price_range != 0:
                ...
                alarm = Alarm(list_id, coin, pair, upper_price, _type, 'Rises above')
                ...
                alarm = Alarm(list_id, coin, pair, lower_price, _type, 'Drops below')
                self.alarms = pd.concat([self.alarms, pd.DataFrame([asdict(alarm)])], ignore_index=True)
                self.create_and_append_alarm_string(alarm)
    ...
    def cancel_order(self):
        try:
            order = self.orders_dict[self.ordersListWidget.currentRow() - 1]
            response = binance_api.cancel_order(order.symbol, order.order_id, self.api_params)
        ...
        try:
            ...
            for key, value in new_balances.items():
                try:
                    ...
                    item = QListWidgetItem(f"{key + ':':<8}{str(current_value) + '$':<13}{str(amount):<13}")
    ...
    def initialize(self):
        try:
            ...
            self.alarms = self.configuration['alarms'] = pd.DataFrame(
    ...
    def track(self, track_dict):
        ...
        self.trackListListWidget.addItem(f"{'COIN':<15} {'PRICE':<15} {'PRC CHANGE':<15} {'LOW PRICE':<15} {'OPEN PRICE':<15} {'HIGH PRICE':<15}")
