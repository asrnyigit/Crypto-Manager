...
class MyMainWindow(QtWidgets.QMainWindow, gui.Ui_MainWindow):
    ...
    def set_order(self, symbol, side, _type, price, quantity):
        try:
            response = binance_api.set_order(symbol, side, _type, price, quantity, self.api_secret, self.api_key).json()
    ...
    def query_balances(self):
        try:
            ...
            self.balancesListWidget.addItem(f"{'COIN':<8} {'VALUE':<8} {'QUANTITY':<8}")
